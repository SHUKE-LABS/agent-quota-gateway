// Package auto implements the `auto` backend selector: a single global
// sticky pointer into the configured backend pool, with reactive 429
// failover and zero synthetic probes.
//
// The design (locked in issue #24) prioritizes stickiness so Anthropic's
// per-account prompt cache is preserved: ride one backend until it
// actually returns a 429, then switch. Nothing is probed — a backend's
// quota is learned only from the real responses organic traffic produces,
// which also keeps each account's rolling 5h window anchored to its own
// first use so resets stay naturally staggered.
//
// State lives entirely in the Controller (in process memory, like the
// quota store). There is no on-disk state and no background goroutine:
// the sticky pointer only moves on a request path (resolution or an
// upstream 429), all under one mutex.
package auto
...
// when the auto selector switches backends. It is deliberately short: the
// switch is instantaneous server-side, so the client should retry almost
// immediately and rebuild its cache on the new backend.
const switchRetryAfterSeconds = 1

// Controller is the global-sticky auto selector. The zero value is not
// usable; call NewController.
type Controller struct {
	mu sync.Mutex

	reg   *backend.Registry
	nicks []string // the pool, in stable sorted order; len >= 1

	// cur indexes nicks: nicks[cur] is currentAuto, the backend every
	// auto request sticks to until it 429s.
	cur int
...
// NewController builds the auto selector over the backends in reg. When
// start < 0 the initial sticky backend is chosen at random (the spec's
// rotating start index — no probe, so any starting point is equally
// valid); otherwise start selects the index deterministically (used by
// tests). now defaults to time.Now and logOut to os.Stderr when nil.
func NewController(reg *backend.Registry, start int, now func() time.Time, logOut io.Writer) *Controller {
	if now == nil {
		now = time.Now
	}
	if logOut == nil {
	...
	nicks := reg.Nicks() // sorted; backend.Load guarantees at least one
	c := &Controller{
		reg:       reg,
		nicks:     nicks,
...
// ResolveAuto returns the backend an `auto` request should use now. It
// implements backend.AutoResolver. When the whole pool is exhausted it
// returns exhausted=true with the soonest-resetting backend and the wait
// until that reset; the caller emits an honest 429.
...
// Current returns the nick of the active sticky backend, for the quota
// view's active_backend field.
...
// ModifyResponse is the proxy.ResponseModifier hook. It only acts on an
// auto-routed request that drew an upstream 429; everything else passes
// through untouched. On such a 429 it records the backend's reset window,
// advances currentAuto, and either rewrites the response to a 503 (a
// healthy backend remains — the client retry will succeed there) or
// forwards an honest 429 with a precise Retry-After (the pool is dry).
func (c *Controller) ModifyResponse(resp *http.Response) error {
	if resp == nil || resp.Request == nil {
		return nil
	}
	if !backend.IsAutoRequest(resp.Request.Context()) {
		return nil
	}
	...
	if res.allExhausted {
		...
		fmt.Fprintf(c.logOut, "auto: all backends exhausted; forwarding 429 (retry after %ds)\n", secs)
		return nil
	}

	if res.switched {
		fmt.Fprintf(c.logOut, "auto: %s -> %s (%s hit 429)\n", b.Nick, res.to, b.Nick)
	}
	rewriteTo503(resp)
	return nil
}
...
type record429Result struct {
	...
	switched     bool          // whether currentAuto actually moved
...
// record429 marks nick exhausted until reset and advances currentAuto if
// needed. It only rotates when the current sticky backend is itself
// exhausted: under concurrent 429s on the same backend, the first call
// rotates and later calls see an already-healthy sticky pointer and leave
// it put, so stickiness is not eroded by redundant hops. When every
// backend is exhausted it points currentAuto at the soonest to reset.
...
// registry, so Resolve always succeeds.
func (c *Controller) backendAt(i int) backend.Backend {
	b, _ := c.reg.Resolve(c.nicks[i])
	return b
}
...
// rewriteTo503 turns an upstream 429 into the transient 503 the auto
// selector hands a client during a switch. The body is replaced with a
// small JSON object, Retry-After invites an almost-immediate retry, and
// the upstream rate-limit headers are stripped so the synthetic response
// does not carry the rejected backend's quota state out the auto channel.
